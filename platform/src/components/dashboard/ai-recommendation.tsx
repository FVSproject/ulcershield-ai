...
export function AiRecommendation() {
  ...
  const evaluateStatus = useEvaluateStore((s) => s.status);
  const startVerify = useEvaluateStore((s) => s.startVerify);
        ...
        >
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <Button
              ...
              onClick={apply}
              disabled={applied || evaluateStatus === "waiting"}
            ...
            {evaluateStatus === "waiting" && (
              <span className="text-[11px] text-[var(--c-muted)]">
                {t("evaluate_running_hint")}
              </span>
            )}
          </div>
        </motion.div>
